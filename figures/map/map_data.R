...
## Load map data
strata <- read_sf("figures/map/DFO_NL_survey_strat/2HJ3KLNOP_Strata_Polygons_WGS84.shp", 
...
nl <- read_sf("figures/map/NL_shapefile/NL.shp", layer = "NL")
nl_utm <- st_transform(x = nl, crs = 32622) # utm zone 22
na <- read_sf("figures/map/north_america_bathy/bathy.shp", layer = "bathy")
na_utm <- st_transform(x = na, crs = 32622)
na_land <- read_sf("figures/map/north_america/bound_p.shp", layer = "bound_p")
na_land <- na_land[na_land$COUNTRY != "water/agua/d'eau", ]
nafo <- read_sf("figures/map/NAFO_divisions/Divisions.shp", layer = "Divisions") # downloaded these data here: http://www.nafo.int/about/frames/area.html
...
bathy <- raster("figures/map/GEBCO/NAFO_GEBCO_data.nc")  # downloaded these data here: http://www.gebco.net/
bathy_utm <- projectRaster(bathy, crs = CRS("+proj=utm +zone=22 +datum=WGS84 +units=m +no_defs"))

fall_transects <- read_sf("figures/map/1990sAcoustictransects/NAFC_Fall_Acoustic_survey.shp", 
                          layer = "NAFC_Fall_Acoustic_survey")
fall_transects <- st_transform(fall_transects, crs = 4326)
ussr_transects <- read_sf("figures/map/1990sAcoustictransects/USSR_Fall_Acoustic_Survey.shp", 
                          layer = "USSR_Fall_Acoustic_Survey")
ussr_transects <- st_transform(ussr_transects, crs = 4326)
spring_ussr_transects <- read_sf("figures/map/1990sAcoustictransects/Bakanev1992_CapelinSurveyTrack.shp", 
...
raw_sp_transects <- read.csv("figures/map/mostly_clearn_cruise_track.csv")

cap <- read.csv("figures/map/Fall_MS_catches_of_capelin_all_set_types_1978-2016.csv")
